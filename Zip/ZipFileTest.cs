...
// 5. tested on Android ,IOS, Mac , Windows. 

// *****
// Include an example : 
// first : download zip file from StreamingAssets folder to local file
// second : extract the zip file (include some textures)
// third : use WWW load the local textures and show them Up.(Use NGUI's UIPopList to choose which one should be load.)
// *****

// Cool Chinese.
// 使用SharpZipLib在Unity中压缩和解压文件
// 1. 注意多平台文件路径
// 2. 压缩文件和文件夹
// 3. 解压文件到指定文件夹
// 4. 在Mac系统中压缩问价会携带一些额外的文件，做一个简单的处理
// 5. 在多平台测试

// 包括一个简单的例子，从StreamingAsserts下载zip文件到本地，解压缩文件(包含了一些图片资源)
// 使用WWWW 加载图片显示在游戏中 使用NGUI的UIPopList选择需要加载的图片
